...
# Prevent curl from hanging if Canton/validator is slow or unreachable (Docker healthcheck has limited timeout)
...
get_admin_token() {
  ...
  curl -f -s -S --connect-timeout "$CURL_CONNECT_TIMEOUT" --max-time "$CURL_MAX_TIME" "${tokenUrl}" \
